...
public class MainActivity extends AppCompatActivity {
    ...
    static final int MENUITEM_CYAM=3;
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        Log.d("myapp", "onCreateOptionsMenu - 최초 메뉴키를 눌렀을 때 호출됨");

        // #2 심지어 메뉴 아이템을 동적으로 추가 가능하다.
        //add(), MenuItem 리턴

        MenuItem item1 = menu.add(1,MENUITEM_YELLOW,100,"노랑");
        MenuItem item2 = menu.add(1,MENUITEM_ORANGE,100,"오렌지");
        MenuItem item3 = menu.add(1,MENUITEM_CYAM,100,"파랑");
        return true; //false를 리턴하면 메뉴는 보여지지 않는다.
    }
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        Log.d("myapp","onPrepareOptionsMenu - 옵션메뉴가 화면에 보여질 때 마다 호출됨");

        //메뉴가 보여질 때 마다 '로그인'과 '로그아웃'이 enable/disable 토글되어 보이기
        if(blog){ //로그인상태
    ...
    public void showInfo(MenuItem item){
        ...
        String title = item.getTitle().toString(); //옵션 메뉴 아이템의 title값
        ...
        Log.d("myapp",msg);
        Toast.makeText(this, title+"메뉴클릭", Toast.LENGTH_SHORT).show();
