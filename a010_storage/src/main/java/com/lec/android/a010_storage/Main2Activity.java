...
public class Main2Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        &&checkSelfPermission(String.valueOf(PERMISSIONS))== PackageManager.PERMISSION_DENIED){
            requestPermissions(PERMISSIONS,REQUEST_CODE);//권한 요청
        ...
        btnSave.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(!checkExternalStorage())return;//외부메모리 사용 못하면 종료
...
//                Environment.getExternalStoragePublicDirectory();//API29 부터 deprecated됨
    ...
    protected boolean checkExternalStorage(){
        String state = Environment.getExternalStorageState();//외부장치가 있는지 확인

        String msg = "";
        //외부 메모리 상태
        if(Environment.MEDIA_MOUNTED.equals(state)){ //mounted
            //읽기, 쓰기 모두 가능한 상태
            msg = "외부메모리 쓰기, 읽기 모두 가능";
            Log.d("myapp", msg);
            ...
            //읽기 쓰기 모두 안됨
            msg = "외부메모리 읽기 쓰기 모두 안됨 / state : " + state;
            Log.d("myapp",msg);
            Toast.makeText(this, msg, Toast.LENGTH_SHORT).show();
            return false;
