...
public class Main2Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //방법3
        BackThread3 thread3 = new BackThread3(handler3); //메인 스레드의 Handler 객체를 외부 클래스에 넘겨줌
    ...
    class BackThread1 extends Thread{
        ...
        public void run() {
            while(true){
                //방법 1) 메인에서 생성된 Handler 객체의 sendEmptyMessage 를 통해 Message전달
                backValue1++;
                handler1.sendEmptyMessage(1);

                //방법 2) 메인에서 생성된 Handler 객체의 postXXX()를 통해 Runnable 객체 전달
                backValue2 +=2;
    ...
    Handler handler1 = new Handler(){ //익명클래스 생성성
       @Override
        public void handleMessage(@NonNull Message msg) {
            if(msg.what==1) { //msg.what-->msg의 아이디
...
//방법 4 : 메인스레드의 Handler 를 직접 사용할 수 없는 분리된 작업 스레드
