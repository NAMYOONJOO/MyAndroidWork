...
public class Main5Activity extends AppCompatActivity {
    ...
    class BackgroundTask extends AsyncTask<Integer,Integer,Integer>{
        ...
        //백그라운드 작업, 반드시 구현
        @Override
        protected Integer doInBackground(Integer... integers) { //가변 매개변수 , integers 는 Integer[]
        ...
        //백그라운 작업 도중(여러번) 호출 가능, 진행사항 업데이트, 중간중간 UI 업데이트시 사용
        ...
        protected void onProgressUpdate(Integer... values) {
            ...
            tvBackValue1.setText("onProgressUpdate"+values[0]); // ★ Handler없이도 메인 UI 접근 가능
        }
        //doInBackground() 완료되면 호출


        @Override
        protected void onPostExecute(Integer integer) { //doInBackGround 에서 return한 값을 매개변수로 받는다.
            Log.d("myapp", "Result : "+integer);
            super.onPostExecute(integer);

            tvBackValue1.setText("onPostExecute:"+integer); //★ Handler없이도 메인 UI 접근 가능
        }
    }
