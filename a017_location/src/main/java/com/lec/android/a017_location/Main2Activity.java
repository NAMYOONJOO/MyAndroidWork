...
public class Main2Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnLocate.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if(btnLocate.isChecked()) {//토글버튼
                    ...
                    try {
                        // 주어진 위치 제공자가 기억하는 가장 최근의 위치정보 획득, 없으면 null리턴
    ...
    class MyListener implements LocationListener {
        ...
        public void onLocationChanged(Location location) {
            ...
            Toast.makeText(getApplicationContext(),"위치정보갱신!",Toast.LENGTH_LONG).show();
        }
        // 위치제공자 가용하게 될 때
        @Override
        public void onProviderEnabled(String provider) {
            Log.d("myapp","onProviderEnable,provider :"+provider);
        }
        //위치 제공자 가용하게 될 때
        @Override
        public void onProviderDisabled(String provider) {
            Log.d("myapp","onProviderDisable,provider :"+provider);
        }
        //API29부터 deprecated 됨 (신경 x)
        @Override
        ...
        switch (requestCode){
            case REQUEST_CODE:
                if(grantResults.length>0&&grantResults[0]==PackageManager.PERMISSION_GRANTED) {
                    ...
                    Toast.makeText(this, "위치권한 실패!", Toast.LENGTH_SHORT).show();
                }
        }
    }
}//end Activity
