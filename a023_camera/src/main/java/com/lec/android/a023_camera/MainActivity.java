...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //권한 획득
        if(Build.VERSION.SDK_INT>=23){ //23버전이상이고
            if(checkSelfPermission(String.valueOf(permission))== PackageManager.PERMISSION_DENIED){
                //현재 이 권한들에대한 권한을 아직 승인받지못하였다면
                requestPermissions(permission, REQUEST_CODE); //권한 요청하기
    ...
    public void takePicture(){
        ...
        Uri fileUri = FileProvider.getUriForFile(this, "org.techtown.capture.intent.fileprovider"
        , file);
        ...
        if(intent.resolveActivity(getPackageManager())!=null){//사진촬영기능이 있는지 여부 확인
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if(requestCode==101&&resultCode==RESULT_OK){
            // 저장된사진  이미지 파일을 ->Bitmap 객체 ->ImageView에 띄우기
